export default function CallOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0b1220] flex flex-col">
        ...
        {isVideo ? (
          <>
            ...
            <video
              ...
              playsInline
              muted
              className="absolute bottom-4 right-4 w-40 h-28 rounded-xl object-cover border-2 border-slate-600 shadow-lg"
            />
          </>
        ) : (
      ...
      <div className="flex items-center justify-center gap-4 p-6 bg-[#1e293b]/80 backdrop-blur">
        <button
          onClick={onToggleMute}
          className={`p-4 rounded-full transition ${isMuted ? 'bg-red-500/20 text-red-400' : 'bg-slate-700 text-white hover:bg-slate-600'}`}
        ...
        {isVideo && (
          <button
            ...
            className={`p-4 rounded-full transition ${isVideoOff ? 'bg-red-500/20 text-red-400' : 'bg-slate-700 text-white hover:bg-slate-600'}`}
